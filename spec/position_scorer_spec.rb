...
describe Position_Scorer do
	...
	it "returns 1 for a winning position" do
		...
		Position_Scorer.return_score(board, 'X').should == 1
	end
    ...
    board.set_marker('O', 'B3')
		Position_Scorer.return_score(board, 'O').should == 1
	end
	...
	it "returns -1 for a losing position" do
		board.set_marker('O', 'B3')
		board.set_marker('O', 'B2')
		Position_Scorer.return_score(board, 'A1').should == -1
	end

	it "returns -1 for a losing position" do
		board.set_marker('O', 'B3')
		board.set_marker('O', 'B2')
		Position_Scorer.return_score(board, 'A3').should == -1
	end
